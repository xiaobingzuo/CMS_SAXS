...
function varargout = qCalibration_BNL(varargin)
...
% Last Modified by GUIDE v2.5 31-Oct-2022 12:08:28

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @qCalibration_BNL_OpeningFcn, ...
                   'gui_OutputFcn',  @qCalibration_BNL_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function qCalibration_BNL_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = qCalibration_BNL_OutputFcn(hObject, eventdata, handles) 
...
elseif scatt.isPilatus2m ==3    % Mar300
    set(handles.rbMar300, 'Value', 1); 
...
elseif qCalData.scatt.isPilatus2m == 3 % for Mar300 detector
    saxs = qCalData.scatt;
    [filename, pathname] = uiputfile('saxssetup.mat', 'Save file name(*.mat)');
    if filename==0
        return;
...
elseif qCalData.scatt.isPilatus2m == -1 % for other detector
    ...
    if filename==0
        ...
        if contains(lower(filename), 'saxs') 
            saxs.type= 'saxs';
        elseif contains(lower(filename), 'waxs') 
            saxs.type= 'waxs';
        end
        save([pathname, filename], 'saxs');
        disp('setup file is saved!')
...
elseif get(handles.rbMar300,'Value')
...
fnInfo = h5info(fullfile(pathname,file));
...
%     tic
    ...
    for kk=1:nSamp
        ...
        fnPrefix='';
        if imgType=='saxs'
            imgs = h5read(fullfile(pathname, file), [cfn0 '/primary/data/pil1M_image']);
            fnPrefix='S';
            
        elseif imgType=='waxs'
            imgs = h5read(fullfile(pathname, file), [cfn0 '/primary/data/pilW2_image']);
            fnPrefix='W';
            
        else
            imgs = [];
            fnPrefix='';
        Its = h5read(fullfile(pathname, file), [cfn0 '/em2_sum_all_mean_value_monitor/data/em2_sum_all_mean_value']);  
        % in process here here
        end
        if ~isempty(imgs)
            [~,~,nImg] = size(imgs);
            for jj=1:nImg
                ...
                % figure(200); imagesc(mask);
                % figure(201); imagesc(sImg); set(gca,'Clim', [0 800]);  
                % figure(202); loglog(data(:,1), data(:,2));
                
                % data2 = circavgnew2(sImg, flipud(mask), scatt.qCMap, scatt.qRMap, scatt.qArray, scatt.offset, scatt.limits);
                % figure(300); imagesc(flipud(mask));
                % figure(301); imagesc(sImg); set(gca,'Clim', [0 500]);  set(gca, 'YDir','normal');
                % figure(302); loglog(data2(:,1), data2(:,2));


                % scale for waxs
                outputFileName = sprintf('%s%s_%05d.dat', fnPrefix, cfn, jj);
                outputFile = [newAvgFolder outputFileName];
                waxsOutputFile = [pathname 'W' outputFileName(2:end)];  

                if imgType == 'waxs'
                    q=data(:,1); [idx1, idx2] = findQrange(q, 1.9, 2.1);
                    intScaling = trapz(q(idx1:idx2), data(idx1:idx2,2)) / (q(idx2) - q(idx1));
                    imgName = sprintf('%s_%s_pilW2_%05d.tif',file,cfn,jj);
                    imgName2 = sprintf('%s%s_%05d.tif',fnPrefix,cfn,jj);
                elseif imgType == 'saxs'
                    intScaling = getScaleWAXS(waxsOutputFile);
                    imgName = sprintf('%s_%s_pil1M_%05d.tif',file,cfn,jj);
                    imgName2 = sprintf('%s%s_%05d.tif',fnPrefix,cfn,jj);
                else
                    intScaling = 1.0;
                    imgName ='';
                ...
                %dat2 = [q, data(:,2:3)/intScaling];
                if ~isempty(imgName2)
                    imwritetiff2(flipud(sImg), imgName2, 32);
                end
                errmsg ='';
                ...
                if fid5 > 0
                    ...
                    fprintf(fid5, '# Scaled by Water peak: %.4f\n', intScaling);
                    fprintf(fid5, '# \n');
                    fprintf(fid5, '# q_(1/A)    I(q)_(cm^-1)     I(q)_std\n');
                    
                    fclose(fid5);

                    dlmwrite(outputFile,[data(:,1) data(:,2:3)/intScaling ], '-append', 'delimiter','\t','precision','%.6e');
                else
...
% read intensity scaling factor from 1D waxs for BNL data 
function intScaling = getScaleWAXS(waxsOutputFile)
intScaling = 1.0;
fid = fopen(waxsOutputFile);
if fid<0
    sprintf('%s not openned', waxsOutputFile);
return;
end
...
if(fid ~= -1)
    try
        while feof(fid)==0
            line=fgetl(fid);
            if (length(line) > 24 & line(1:23) =='# Scaled by Water peak:')
                    intScaling= str2num(strtrim(line(24:end)));
                    break;
            else
                continue;
            end    
...
%     end
    ...
    if isReady
        ...
        if fid5 > 0            
            ...
            dlmwrite(outputFile,[data(:,1) data(:,2:3)*absIntCoeff/phd], '-append', 'delimiter','\t','precision','%.6e');
        else
            disp(sprintf('Error: file: %s cannot open! %s.\n', outputFile, errmsg));
            %disp(errmsg);
        end   
...
figure('NumberTitle','off','Name',cfile);imagesc(sImg, [0 500]); set(gca, 'YDir','normal');
...
if ~sectorMaskOn
    ...
    row = scatt.row; col = scatt.col;
    if (row ~= imgHrow || col ~= imgHcol)
        disp('image size not match! use qCalibration_BNL parameters!');
    end
